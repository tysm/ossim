...
auto MemoryManager::run() -> std::unique_ptr<Process> {
  if (process) {
    ...
    while (!alloc_buffer.empty() && victim_page == size_t(-1) &&
...
void MemoryManager::pagination() {
  ...
  auto it = swap.begin();
  while (it != swap.end() &&
         ((*it).first != process->pid || (*it).second != ref.second))
    it++;
  // Then remove it
  if (it != swap.end())
    swap.erase(it);
...
bool MemoryManager::try_alloc(
    unsigned pid, std::shared_ptr<vector_pair<size_t, size_t>> page_refs,
    bool checking) {
